...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const cartIcon = document.querySelector('.navRightItemsImages img[src="./assets/images/Cart1.png"]');
    ...
    addToCartButtons.forEach(button => {
      button.addEventListener('click', function () {
        ...
        const itemImage = card.querySelector('.cardImage img').getAttribute('src');
        const itemPrice = parseFloat(card.querySelector('.newPrice').innerText.replace('$', ''));
  
        // Create an item object
        const item = { name: itemName, image: itemImage, price: itemPrice, quantity: 1 };
